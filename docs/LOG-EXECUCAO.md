...
- Preflight de registries: PyPI `decision-maker` **livre** (404) ✓; npm `decision-maker` **ocupado por terceiro** (`asafamir`, desde 2022) → decisão do usuário: pacote npm = **`@gabedsam01/decision-maker`** (escopado, `--access public`), PyPI segue `decision-maker`. Secret `NPM_TOKEN` criado e confere com o nome esperado pelo job; `PYPI_TOKEN` criado pelo usuário não é usado (OIDC cobre) — mantido como reserva.
